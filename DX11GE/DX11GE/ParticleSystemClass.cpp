...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	for(int i=0; i<m_maxParticles; i++)
	{
		...
		m_particleList[i].dir.x == 0;
		m_particleList[i].dir.y == 0;
	}
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		for (;;)
		{
			/*dirX = float(rand() % 3) - 1;
			dirY = float(rand() % 3) - 1;*/
			dirX = ((double)rand()* (double)3 / (((double)RAND_MAX) + (double)(1))) - 1;
			dirY = ((double)rand()* (double)3 / (((double)RAND_MAX) + (double)(1))) - 1;
			if (!(dirX == 0 || dirY == 0))
				break;
		}
		...
		while(i != index)
		{
			...
			m_particleList[i].dir.x = m_particleList[j].dir.x;
			m_particleList[i].dir.y = m_particleList[j].dir.y;
			i--;
			j--;
		}
		...
		m_particleList[index].dir.x = dirX;
		m_particleList[index].dir.y = dirY;
	}
}
...
void ParticleSystemClass::UpdateParticles(float frameTime)
{
	...
	{
		m_particleList[i].positionY = m_particleList[i].positionY - (m_particleList[i].velocity * frameTime * 0.001f* m_particleList[i].dir.y);

		m_particleList[i].positionX = m_particleList[i].positionX - (m_particleList[i].velocity * frameTime * 0.001f* m_particleList[i].dir.x);
	}
}
...
void ParticleSystemClass::KillParticles()
{
	...
	for(int i=0; i<m_maxParticles; i++)
	{
		if((m_particleList[i].active == true) && (m_particleList[i].positionY < -1.f|| m_particleList[i].positionY > 1.f || m_particleList[i].positionX < -1.f || m_particleList[i].positionX > 1.f))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
			...
			for(int j=i; j<m_maxParticles-1; j++)
			{
				...
				m_particleList[j].dir.x = m_particleList[j + 1].dir.x;
				m_particleList[j].dir.y = m_particleList[j + 1].dir.y;
			}
		}
	}
}
