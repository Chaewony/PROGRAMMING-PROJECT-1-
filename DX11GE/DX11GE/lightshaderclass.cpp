...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "ToonPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	// Create the pixel shader from the buffer.
	result = device->CreatePixelShader(toonShaderBuffer->GetBufferPointer(), toonShaderBuffer->GetBufferSize(), NULL, &m_toonShader);
	if (FAILED(result))
	{
		return false;
	}
